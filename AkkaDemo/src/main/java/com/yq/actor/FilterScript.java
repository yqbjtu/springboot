...
public class FilterScript extends AbstractActor {

    private LoggingAdapter log = Logging.getLogger(getContext().getSystem(), this);

    static public Props props(String message, ActorRef createAlarmActor, ActorRef clearAlarmActor) {
        return Props.create(FilterScript.class, () -> new FilterScript(message, createAlarmActor, clearAlarmActor));
    }
    ...
    public FilterScript(String message, ActorRef createAlarmActor, ActorRef clearAlarmActor) {
    ...
    public Receive createReceive() {
        return receiveBuilder()
                .match(DeviceDataEvent.class, deviceDataEvent -> {
                    ...
                    if (result) {
                        createAlarmActor.tell(new CreateAlarmAction.AlarmMessage(deviceDataEvent.deviceId, deviceDataEvent.deviceName), getSelf());
                    }
                    else {
                        clearAlarmActor.tell(new ClearAlarmAction.ClearAlarmMessage(deviceDataEvent.deviceId, "ruleId"), getSelf());
                    }
                })
                .build();
    }
